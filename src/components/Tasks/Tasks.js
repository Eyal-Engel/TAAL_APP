...
function Tasks(props) {
    ...
    const getInitialLocation = () => {
        return screen.width > 1020 ? (current_tasks_list.length - 1 - (task_current_index || 0))
            : task_current_index
    }
    ...
    const handleSwipe = useSwipeable({
        onSwiped: () => {
            ...
            if (screen.width > 1020) {
                if (currIndex === 0) {
                    props.actions.completeTask(allData[currIndex].id, allData.length - 1 - currIndex)
                    setModalOpen(true)
                }
                else
    ...
    const markTaskAsFinished = (index) => {
        if (allData[index] && allData[index].id)
            props.actions.completeTask(allData[index].id, screen.width > 1023 ? (allData.length - 1 - index) : index)
    }
    const updateCurrentTask = (nextIndex) => {
        props.actions.changeCurrentTask(allData[nextIndex].title.rendered, screen.width > 1023 ? (allData.length - 1 - nextIndex) : nextIndex)
    ...
    useEffect(() => {
        ...
        if (screen.width > 1020)
            ...
            setAllData(props.user_tasks.current_tasks_list)
    ...
    */
    ...
    const ipadSettings = {
        ...
        nextArrow: <BlueArrow arrowDirection={'right'} visible={allData.length - 1 - currIndex !== 0} onClickArrows={(direction) => onClickArrows(direction)} />,
        prevArrow: <BlueArrow arrowDirection={'left'} visible={currIndex !== 0} onClickArrows={(direction) => onClickArrows(direction)} />,
    ...
    const getLineLength = (sh) => {
        return (currIndex === 0 || currIndex === allData.length - 1) ? (0.5 * sh) : (0.8 * sh)
    }
    const decideClassName = (index, currIndex) => {
        if (index === currIndex)
            return "slide activeSlide"
        else if (index === currIndex + 1)
            return presentFooter(currIndex, prevIndex.current, stationsData) === 'center bottomBar low' ? "slide nextSlide opac" : "slide nextSlide"
        else if (index === currIndex - 1)
    ...
    return (
        <div className="Tasks">
            ...
            {screen.width < 1000 ? <div className="containerTasks">
                <div className="center grayBar" >
                    <Text>{allData[currIndex] ? allData[currIndex].stationDetails.name : ""}</Text>
                </div>
                <Connector height={allData.length < 2 ? 1 : getLineLength(screen.height)} top={getLineHeight(currIndex)} />
...

                        {
                            allData.map((item, index) => {
                                return (
                                    ...
                                    <div key={index} className={decideClassName(index, currIndex)}>
                                        <TaskComp
                                            imgUrl={item.acf && item.acf.image ? item.acf.image.url : false}
                                            title={item.title && item.title.rendered.split('&')[0]}
                                            content={item.content && item.content.rendered}
                                            didFinished={item.didFinish}
                                            audioUrl={item.acf && item.acf.audio ? item.acf.audio.url : false}
                                            index={index}
                                            currentIndex={currIndex}
                                            height={screen.height * 0.280}
                                        />
                ...
                <div className={barStatus2}>
                    <Text>{stationsData[currIndex] ? stationsData[currIndex].name : ""}</Text>
                </div>


                <div>
                    <FinishModal modalOpen={modalOpen} userName={props.username}
                ...
                : <div className={"wideContainer"}>
                    ...
                    <div className={"wideInfoArea"} {...handleSwipe}>
                        <div className={"bigImageContainer center"} style={{ backgroundColor: (allData[currIndex] && allData[currIndex].didFinish) ? '#4CB74C' : '#50595C' }}>
                            <div className={"bigImageWhiteContainer"}>
                                <img alt={'big task image'} src={getLargeImage()} style={{ borderRadius: 5 }} />
                            </div>
                        </div>
                        <div className={"taskDetailsContainer"}>
                            <div className={'detailsTextContainer'}>
                                <Text fontSize={4} textAlign={'right'}>{allData[currIndex] && parseContent(allData[currIndex].title.rendered)}</Text>
                                <Text fontSize={2.5} textAlign={'right'}>{allData[currIndex] && parseContent(allData[currIndex].content.rendered)}</Text>
                            </div>
                            <div className={"audioContainer"}>
                                <AudioIcon width={25} containerStyle={{ alignSelf: 'center' }} />
                            </div>
                        </div>
                    </div>
                    <div className={"wideSliderArea"}>
                        <div style={{ opacity: stationsData.hasOwnProperty(allData.length - 1 - currIndex) ? '1' : '0' }} className={'stationBox gray'}>
                            <Text className={'stationText left'} textAlign={'left'} fontSize={2}>:התחנה הבאה {getStationName(currIndex - 1)}</Text>
                        ...
                        <div className={'stationBox'}>
                            <Text className={'stationText right'} textAlign={'right'} fontSize={2}>{getStationName(currIndex)}</Text>
                            <div className={'stationLogo'}>
                                <Text fontSize={2}>לוגו</Text>
                            </div>
                        </div>
