...
export default function TaskComp(props) {
    ...
    return (
        ...
        <ContainerWrapper height={props.height} width={props.wrapperWidth ? props.wrapperWidth : 50}>
            <TaskContainer
                backgroundColor={decideColor(props.didFinished)}
                width={props.width ? props.width : 50}
                height={props.wideCaro ? props.wideCaro : (isFocused() ? 100 : 60)}
            >
                <WhiteContainer width={isFocused() && props.wideCaro === undefined ? 90 : 80} height={isFocused() && props.wideCaro === undefined ? 90 : 80}>
                    <ImageWrapper
                        maxHeight={isFocused() && props.wideCaro === undefined ? 58 : 100}
                        borderWidth={isFocused() && props.wideCaro === undefined ? 1 : '0'}

                    >
                        <img alt={'task image'} src={props.imgUrl ? props.imgUrl : "https://globalimpactnetwork.org/wp-content/themes/globalimpact/images/no-image-found-360x250.png"} style={styles.imgStyle} />
                    </ImageWrapper>
                    <TopBar display={isFocused() && props.wideCaro === undefined ? 'flex' : 'none'}>
                        <div style={{ width: '20%', height: 'auto', marginTop: 'auto' }}>
                            <AudioIcon audioUrl={props.audioUrl} />
                        </div>
                        <div style={{ height: '100%', width: '79.5%', padding: 0 }}>
                            <Text fontSize={3.5}>xcdc dffv dfvfdvd fdvdvdf{props.title}</Text>
