...
export const extractPathForSite = (userTasks, siteID, user_tasks_total = null) => {
    ...
    userTasks.forEach(currentTask => {
        ...
        if (currentTask.places.includes(parseInt(siteID))) {
            ...
            user_tasks_total ? currentTask["didFinish"] = user_tasks_total[currentTask.id].didFinish : currentTask["didFinish"] = null
            listForExec[index] ? listForExec[index].push(currentTask) : listForExec[index] = [currentTask]
        }

    })
    if (!listForExec[0])
        listForExec = listForExec.slice(1)

    return [listForExec, cleanList]
}


export const getUserTasksFromRouteList = (data, userID) => new Promise((resolve, reject) => {
