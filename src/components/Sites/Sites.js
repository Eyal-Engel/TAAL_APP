...
export default function Sites(props) {
        ...
        if (!(scanResult === 'error')) {
            ...
            if (placesList.hasOwnProperty(scanResult) || placesList.length === 0) {
                if (temp.hasOwnProperty(scanResult)) {
                    let [separateList, cleanList] = extractPathForSite(props.user_tasks.user_tasks, scanResult, trasformObject(props.user_tasks.user_tasks))
                    ...
                    if (placesList[scanResult].acf["defaultPath"]) {
                        let lst = getTasksList(taskInformation, routesInfo[placesList[scanResult].acf["defaultPath"][0].ID].acf.tasks)
                        props.actions.changeCurrentTasks(extractPathForSite(lst, scanResult))
                        setScanning(false)
    ...
    */
    const breakPoints = [
        ...
        { width: 900, verticalMode: false, itemsToShow: 4, itemsToScroll: 2, isRTL: true },
    ]
    ...
    const getDateInfo = () => {
        const date = new Date()
        dateRef.current = date.getFullYear().toString() + '-'
    ...
    useEffect(() => {
        ...
        if (user.user.lastFetchDate !== getDateInfo()) {
            ...
            axios.get(wpConfig.getPlaces, {
                params: {
                    per_page: 70, 'Cache-Control': 'no-cache'
                },
                headers: {
                        ...
                        .then(res => {
                            ...
                            axios.get(wpConfig.getRoutes, {
                                ...
                                .then(async res => {
                                    ...
                                    let userRoutes = await getUserTasksFromRouteList(res.data, user.user.id)
    ...
    const isCurrentSite = (itemId) => (itemId === props.user_places.places_location)

    const getFirstItemLocation = () => {
        if (props.user_places.places_location !== -1)
            return props.user_places.places_location
        else
            return (props.user_places.user_places.length - 1)
    }
                                        ...
                                        props.user_places.user_places.map((item, index) => {
                                            ...
                                            return (
                                                <SiteComp
                                                    key={item.id}
                                                    name={item.name}
                                                    imgUrl={(item.acf && item.acf.image) ? item.acf.image.url : ''}
                                                    didVisit={isCurrentSite(index) ? "current" : item.didVisit}
                                                    onClick={() => console.log(item)}
                                                    audioUrl={(item.acf && item.acf.audio) ? item.acf.audio.url : ''}
                                                />
                                            )
                                        })
                                    }
