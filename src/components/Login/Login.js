...
class Login extends React.Component {
					...
					axios.get(wpConfig.getUser, {
						...
						.then(res => {
							...
							this.props.actions.changeUser({
								imgPath: (extraData.image && extraData.image.url) ? extraData.image.url : null,
								username: user_nicename,
								isLoggedIn: true,
								id: user_ID,
								phone: extraData.phone || '',
								arabicName: extraData.arabic_name || '',
								guideName: (extraData.guide && extraData.guide.display_name) ? extraData.guide.display_name : '',
								hebrewName: res.data.name,
								GuidPhone: (extraData.guide && extraData.guide.user_description) ? extraData.guide.user_description : ''
							});
	...
	render() {
		...
		if (loggedIn || localStorage.getItem('token')) { //if we get the token
			...
			return (
				<div className=" centered">
					...
					<form onSubmit={this.onFormSubmit}>
						...
						<br />
						<button className="btn mb-3" type="submit">התחברות  /  دخول </button>

						{/*{ loading && <img className="loader" src={Loader} alt="Loader"/> }*/}
					</form>
					<button className="forgotPass" onClick={(e) => this.setState({ isOpen: true })}>שכחת סיסמא? نسيت كلمة المرور</button>
					<Modal isOpen={this.state.isOpen} onRequestClose={this.toggleModal} style={this.styles.modalStyle}>
						<div className="popup">
